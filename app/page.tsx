...
export default function HomePage() {
  return (
    <div className="flex flex-col items-center">
      ...
      <section className="w-full relative flex flex-col md:flex-row items-center justify-between gap-8 md:gap-10 pt-6 sm:pt-8 md:pt-12 pb-12 sm:pb-16 md:pb-20 px-4 sm:px-6 md:px-12 max-w-6xl mx-auto min-h-[60vh]">
        <HeroCanvas />
        <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left z-10 w-full relative">
          <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-surface-900/50 backdrop-blur-md border border-brand-500/20 text-brand-400 text-[11px] sm:text-xs md:text-sm font-semibold mb-4 sm:mb-6 md:mb-8 animate-fade-in opacity-0">
          ...
          <h1 className="animate-fade-in opacity-0 text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight text-white mb-3 sm:mb-4 md:mb-6 leading-tight">
            Build Your Resume <br className="hidden md:block"/> in <span className="text-brand-400">Seconds</span>
          </h1>
          ...
          <Link 
            href="/resume-builder"
            className="animate-fade-in opacity-0 animate-delay-200 inline-flex items-center justify-center gap-2 px-6 py-3 md:px-8 md:py-3.5 text-sm sm:text-base md:text-lg font-bold text-surface-950 bg-white rounded-xl sm:rounded-2xl hover:bg-surface-100 transition-all transform hover:-translate-y-1 shadow-[0_4px_20px_rgba(255,255,255,0.15)] hover:shadow-[0_8px_30px_rgba(255,255,255,0.25)] w-full sm:w-auto"
          >
            Create Resume
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
          {pdfTools.map((tool, i) => (
            <Link
              key={tool.title}
              href={tool.href}
              className="bg-surface-800/20 backdrop-blur-md border border-surface-700/50 hover:border-surface-600/60 hover:bg-surface-800/40 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 group transition-all animate-slide-up opacity-0"
              style={{ animationDelay: `${(i + 2) * 50}ms` }}
              id={`tool-card-${tool.title.toLowerCase().replace(/ /g, "-")}`}
            >
              <div
                className={`inline-flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-br ${tool.gradient} shadow-lg text-white mb-3 sm:mb-4 transition-transform group-hover:scale-110 group-hover:shadow-brand-500/20`}
              >
                {tool.icon}
              </div>
              <h3 className="text-sm sm:text-lg font-heading text-surface-50 mb-1 sm:mb-2 tracking-tight">
                {tool.title}
              </h3>
              <p className="text-surface-400 text-xs sm:text-sm line-clamp-3 leading-relaxed font-sans">
                {tool.description}
              </p>
            </Link>
          ))}
