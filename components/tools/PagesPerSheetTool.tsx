...
export default function PagesPerSheetTool() {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {files.length > 0 && (
        <div className="bg-slate-900/30 rounded-2xl p-6 sm:p-8 border border-white/[0.05] animate-fade-in flex flex-col gap-8">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <h4 className="text-sm font-semibold text-emerald-400 uppercase tracking-wider">Layout</h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <label className="text-sm font-medium text-slate-300">Sheet Size</label>
                  <select
                    value={sheetSize}
                    onChange={(e) => setSheetSize(e.target.value as SheetSize)}
                    className="w-full bg-slate-800 border border-slate-700 text-slate-300 text-sm rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all cursor-pointer appearance-none"
                    style={{ backgroundImage: `url('data:image/svg+xml;utf8,<svg fill="none" stroke="%2394a3b8" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>')`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 1rem center', backgroundSize: '1.2em' }}
                  >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex bg-slate-800/50 rounded-lg p-1 border border-slate-700 h-10">
            ...
            {/* Formatting & Style */}
            <div className="space-y-6">
              <h4 className="text-sm font-semibold text-emerald-400 uppercase tracking-wider">Spacing & Order</h4>

              <div className="space-y-3">
                <label className="text-sm font-medium text-slate-300">Reading Direction</label>
                <div className="flex bg-slate-800/50 rounded-lg p-1 border border-slate-700">
                  <button
                    onClick={() => setDirection("row")}
                    className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium transition-all ${
                      direction === "row" ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
                    }`}
                  >
                    Row by Row
                  </button>
                  <button
                    ...
                    className={`flex-1 py-1.5 px-3 rounded-md text-sm font-medium transition-all ${
                      direction === "col" ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"
                    }`}
                  >
                    Column by Column
                  </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-3">
                  <label className="text-sm font-medium text-slate-300">Outer Margins (pts)</label>
                  <div className="grid grid-cols-2 gap-2">
                    <input type="number" min="0" max="100" value={marginTop} onChange={(e) => setMarginTop(Number(e.target.value))} placeholder="Top" className="bg-slate-800 border-none text-slate-300 rounded-md px-2 py-1 text-xs text-center focus:ring-1 focus:ring-emerald-500 outline-none" title="Top" />
                    <input type="number" min="0" max="100" value={marginBottom} onChange={(e) => setMarginBottom(Number(e.target.value))} placeholder="Bottom" className="bg-slate-800 border-none text-slate-300 rounded-md px-2 py-1 text-xs text-center focus:ring-1 focus:ring-emerald-500 outline-none" title="Bottom" />
                    <input type="number" min="0" max="100" value={marginLeft} onChange={(e) => setMarginLeft(Number(e.target.value))} placeholder="Left" className="bg-slate-800 border-none text-slate-300 rounded-md px-2 py-1 text-xs text-center focus:ring-1 focus:ring-emerald-500 outline-none" title="Left" />
                    <input type="number" min="0" max="100" value={marginRight} onChange={(e) => setMarginRight(Number(e.target.value))} placeholder="Right" className="bg-slate-800 border-none text-slate-300 rounded-md px-2 py-1 text-xs text-center focus:ring-1 focus:ring-emerald-500 outline-none" title="Right" />
                  </div>
                </div>

                <div className="space-y-3">
                  <label className="text-sm font-medium text-slate-300">Inner Spacing</label>
                  <div className="flex bg-slate-800/50 rounded-lg p-1 border border-slate-700 flex-col gap-1 h-[68px]">
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  <div className="relative flex items-center justify-center">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`w-11 h-6 bg-slate-700 rounded-full transition-colors duration-300 ${showBorder ? "bg-emerald-500" : ""}`}></div>
